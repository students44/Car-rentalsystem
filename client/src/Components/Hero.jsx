      ...
      p-6 rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white shadow-[0px_8px_20px_rgba(0,0,0,0,1)]' >
       ...
       <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
        ...
        <div className='flex flex-col items-start gap-2 '>
          <select name="" id="" required className='py-2 pickup-select'  onChange={(e) => setPickuPLocation(e.target.value)}>    
       ...
       {/* pick-up your date */}
         <div className='flex flex-col items-start gap-2'>
          ...
          <input type="date" name='date' id='pick-update' min={new Date().toISOString().split('T')[0]} 
          className='text-sm text-gray-500'  onChange={(e) => setPickUpDate(e.target.value)} required />
        ...
        {/* return date */}
         <div className='flex flex-col items-start gap-2'>
          ...
          className='text-sm text-gray-500' onChange={(e) => setReturnDate(e.target.value)} required/>
        </div>

        {/* search button */}
        <button type='button' className='flex items-center justify-center gap-1 px-8 py-3 max-sm:mt-4 bg-primary hover:bg-primary-dull
        text-white rounded-full cursor-pointer'  onClick={handleSearchClick}>
